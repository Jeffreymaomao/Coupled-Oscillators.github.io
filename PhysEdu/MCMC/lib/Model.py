...
def Numerical_Model(t, k, m, xi, vi):
    """
    Parameters
    ----------
        t     : <time series>
    ...
    def F(t,U):
        ...
        a1_ = -(k1 * x1_ + k2 * (x1_ - x2_))/m[0]
        a2_ = -(k3 * x2_ + k2 * (x2_ - x1_))/m[1]
...
def histMCMC(Paras, bins=30, labels=None, figsize=(12,9), dpi=200, cmap="jet", title=None):
    fig = plt.figure(figsize=figsize, dpi=dpi)
    if(title):
        plt.title(title)
    N, n = Paras.shape
    ...
    plt.show()
    return {"max":Paras_max, "mean":Paras_mean, "figure":fig}
